...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    // 2. Stats Réservations 
    const { data: resas } = await supabase
      .from('reservations')
      .select('status, total_price, start_at, client_name, service_name, payment_status')
    ...
    if (resas) {
      ...
      // CA Réalisé (Uniquement ce qui est marqué comme payé)
      const actualRev = confirmed
        .filter(r => r.payment_status === 'paid')
        .reduce((acc, curr) => acc + (Number(curr.total_price) || 0), 0);

      const sortedUpcoming = resas
        .filter(r => r.status === 'confirmed' && new Date(r.start_at) >= now)
  ...
  return (
    <>
        <SeoHead 
            title="Tableau de Bord - Saphir Admin"
            description="Gestion des réservations et de l'activité."
        />  
        
        <div className="min-h-screen bg-[#050505] text-white flex flex-col md:flex-row font-sans">
        
        {/* SIDEBAR */}
        <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#0f0f0f] border-r border-white/5 transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="flex flex-col h-full p-6">
            <div className="flex items-center gap-3 mb-10 pb-6 border-b border-white/5 cursor-pointer" onClick={() => navigate('/')}>
                <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center font-black text-black text-lg"><Home className="w-5 h-5" /></div>
                <span className="font-black tracking-tighter text-xl uppercase">Saphir</span>
            </div>

            <nav className="flex-1 space-y-2">
                {menuItems.map((item) => (
                <button key={item.id} onClick={() => { setActiveTab(item.id); setIsSidebarOpen(false); }} className={`w-full flex items-center justify-between px-4 py-3.5 rounded-2xl transition-all ${activeTab === item.id ? 'bg-primary text-black font-black shadow-glow' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                    <div className="flex items-center gap-3">
                    <item.icon className="w-5 h-5" />
                    <span className="text-[10px] font-black uppercase tracking-widest">{item.label}</span>
                    </div>
                    {item.badge > 0 && <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg ${activeTab === item.id ? 'bg-black text-white' : 'bg-primary text-black'}`}>{item.badge}</span>}
                </button>
                ))}
            </nav>

            <button onClick={handleLogout} className="mt-auto flex items-center gap-3 px-4 py-4 text-gray-500 hover:text-red-500 font-bold text-[10px] uppercase tracking-[0.2em] transition-colors border-t border-white/5"><LogOut className="w-5 h-5" /> Déconnexion</button>
            </div>
        </aside>

        {/* MAIN CONTENT */}
        <main className="flex-1 p-4 md:p-10 overflow-y-auto max-w-[1600px] mx-auto w-full">
            ...
            <div className="space-y-8 animate-in fade-in duration-700">
                ...
                <div className="bg-[#0f0f0f] border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                ...
                <div className="bg-[#0f0f0f] border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                    ...
                    <div>
                    <p className="text-3xl font-black text-white uppercase tracking-tight leading-none">{nextChantierTime || "Libre"}</p>
                ...
                <div className="lg:col-span-2 bg-[#0f0f0f] border border-white/5 rounded-[2.5rem] p-8 shadow-2xl">
                    ...
                    {upcoming.map((resa, i) => (
                        ...
                        <div className="flex items-center gap-5">
                            <div className="text-center bg-black px-4 py-3 rounded-2xl border border-white/10">
                            ...
                            <p className="text-2xl font-black leading-none">{new Date(resa.start_at).getDate()}</p>
                            </div>
                            <div>
                            <p className="font-black text-white uppercase text-sm group-hover:text-primary transition-colors">{resa.client_name}</p>
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{resa.service_name}</p>
                            </div>
                        </div>
                        <div className="text-right">
                ...
                <div className="bg-primary/5 border border-primary/20 rounded-[2.5rem] p-8 space-y-8">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">CA Prévisionnel Total</p>
                    </div>
                    
                    <div className="pt-4">
                        <div className="h-3 w-full bg-white/5 rounded-full overflow-hidden mb-2 shadow-inner">
                        <div className="h-full bg-primary shadow-glow transition-all duration-1000" style={{ width: `${Math.min((stats.actualRevenue / stats.totalRevenue) * 100 || 0, 100)}%` }}></div>
                        </div>
                        <p className="text-[9px] font-black text-primary uppercase text-right tracking-[0.2em]">Recouvrement : {Math.round((stats.actualRevenue / stats.totalRevenue) * 100 || 0)}%</p>
                    </div>
                    </div>
                </div>
